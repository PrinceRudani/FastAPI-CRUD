...
class SubcategoryDAO:
    ...
    @staticmethod
    def get_subcategory_by_id_dao(id: int, session):
        """Fetch a single subcategory by ID (excluding soft-deleted records)."""
        return MysqlCommonQuery.get_by_id_query(SubcategoryVO, id, session)

    @staticmethod
    def update_subcategory_dao(subcategory: SubcategoryVO, session):
        """Update an existing subcategory."""
        return MysqlCommonQuery.update_query(subcategory, session)
