...
class LoginService:
    ...
    @staticmethod
    def login_user(login_dto, response: Response, background_tasks):
        ...
        if not bcrypt.checkpw(
            login_dto.login_password.encode(), user.login_password.encode()
        ):
            raise HTTPException(
    ...
    def refresh_token(response: Response, request):
        ...
        try:
            data = jwt.decode(
                refresh_token, Constant.SECRET_KEY, algorithms=[Constant.HASH_ALGORITHM]
            )

            access_token, new_refresh_token = LoginService.generate_tokens(
                data["user_id"], data["username"], user_role=StaticVariables.ADMIN_ROLE
            )
